...
'''eg: python3 htmlReader_Fahlmer.py "../data/briefe-von-goethe-an-johanna-fahlmer" "../ignore/letters/JohannaFahlmer/html" '''
...
def extract(soup, destination):
    ...
    for element in document:
        if element.name == "h3":
            ...
            if title_counter != 0:
                into_html(title, number, date, html_body, destination)
                into_txt(title, number, date, txt_body, destination)
                txt_body = ""
                del html_body[:]

            number = ((element.text).lstrip()).replace("."," ")
            number = roman.fromRoman(str(number))
            title_counter += 1
...
def write_metadata(soup, filename, direc, source):
    ...
    name = filename.replace("ignore/letters/JohannaFahlmer/html/","ignore/letters/JohannaFahlmer/meta/")
    metadata = {}
