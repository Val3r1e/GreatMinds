...
'''
python3 wordcloud_generator.py "../Text/nouns/"
'''

def time_period(direc, name, steps):
    '''
...
def create_wordcloud(text, name, steps, year):
    ...
    wc.generate(text)
    wc.to_file(path.join(currdir, "wordclouds/%s/%s/%s_%s_%s.png" %(name, steps, name, year, steps)))
